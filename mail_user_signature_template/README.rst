...
Templates now include a sanitization checker to preview how your
signature will appear in different email clients:
...
-  **Border shorthand**: Use individual properties
   (``border-left-width``, ``border-left-style``, ``border-left-color``)
-  **Font stacks**: Use single font families instead of comma-separated
   lists
-  **Background shorthand**: Use ``background-color`` instead of
   ``background``
-  **Unsupported properties**: Properties like ``letter-spacing`` and
   ``outline`` are removed

The checker helps ensure your signatures display consistently across all
email clients by identifying styles that will be stripped during email
sending.
