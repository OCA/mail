...
class MailAttachmentOfficePreviewController(http.Controller):
    @http.route(
        "/mail_attachment_office_preview/to_pdf",
        type="http",
        auth="user",
        methods=["GET"],
    )
    ...
    def _get_pdf_cached(self, content, ext):
        ...
        if cache_path and os.path.exists(cache_path):
            try:
                ...
                _logger.warning(
                    "mail_attachment_office_preview: cache read failed", exc_info=True
                )
        ...
        if pdf_bytes and cache_path:
            try:
                ...
                _logger.warning(
                    "mail_attachment_office_preview: cache write failed", exc_info=True
                )
        return pdf_bytes
    ...
    def _cache_path(key):
        ...
        try:
            base = os.path.join(
                ...
                "mail_attachment_office_preview_cache",
            )
    ...
    def _libreoffice_to_pdf(content, ext):
        ...
        if ext in OOXML_SPREADSHEET_EXTENSIONS:
            content = MailAttachmentOfficePreviewController._strip_saved_scroll(content)
        try:
