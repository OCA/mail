...
class TestMailWhatsAppCoverage(MailGatewayTestCase):
    ...
    def test_receive_update_ignores_other_fields(self):
        service = self.env["mail.gateway.whatsapp"]
        token = self.partner.phone.replace("+", "")
        value = {
            "messages": [
                {
                    "from": token,
        ...
        update = {"entry": [{"changes": [{"field": "account_update", "value": value}]}]}
        service._receive_update(self.gateway, update)
        self.assertFalse(
            self.env["discuss.channel"].search([("gateway_id", "=", self.gateway.id)])
        )

    def test_send_payload_document(self):
        service = self.env["mail.gateway.whatsapp"]
        channel = self.partner._whatsapp_get_channel("phone", self.gateway)
        payload = service._send_payload(
            channel,
            media_id="MEDIA_ID",
            media_type="document",
        ...
        self.assertEqual(
            payload["document"], {"id": "MEDIA_ID", "filename": "invoice.pdf"}
        )
        payload = service._send_payload(
            channel, media_id="MEDIA_ID", media_type="image"
        )
        self.assertEqual(payload["image"], {"id": "MEDIA_ID"})
    ...
    def test_whatsapp_get_partner_without_partner_field(self):
        self.assertIsNone(self.env["mail.blacklist"]._whatsapp_get_partner())
    ...
    def test_whatsapp_composer_missing_record(self):
        ...
        self.assertFalse(composer.action_view_whatsapp())
        self.assertFalse(composer.action_send_whatsapp())
        self.assertFalse(
            self.env["discuss.channel"].search([("gateway_id", "=", self.gateway.id)])
        )
