...
class TestMailCcBccInvoice(TestAccountMoveSendCommon, MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partner = env["res.partner"].create(
            {
                "name": "Test CC BCC Invoice Partner",
                "email": "invoice.partner@example.com",
            }
        )
        cls.partner_cc = env["res.partner"].create(
            {"name": "Test CC BCC Cc Partner", "email": "cc.partner@example.com"}
        )
        cls.partner_bcc = env["res.partner"].create(
            {"name": "Test CC BCC Bcc Partner", "email": "bcc.partner@example.com"}
        )
